...
bool Plane::intersect(const Ray &ray, glm::vec3 & point, glm::vec3 & normalAtIntersect)
{
	...
	bool hit = glm::intersectRayPlane(ray.p, ray.d, posVec, this->normal, dist);
	if (hit)
	{
		Ray r = ray;
		point = r.evalPoint(dist);
		normalAtIntersect = this->normal;
		glm::vec2 xrange = glm::vec2(posVec.x - width / 2, posVec.x + width / 2);
		glm::vec2 zrange = glm::vec2(posVec.z - height / 2, posVec.z + height / 2);
