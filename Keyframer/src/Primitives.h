...
public:
	SceneObject() {
		gui.setup();
		gui.add(position.set("Position", this->position, glm::vec3(-20, -20, -20), glm::vec3(20, 20, 20)));
		gui.add(rotation.set("Rotation", this->rotation, glm::vec3(-360, -360, -360), glm::vec3(360, 360, 360)));
		gui.add(scale.set("Scale", this->scale, glm::vec3(-5, -5, -5), glm::vec3(5, 5, 5)));
	}
	virtual ~SceneObject() {}
	...
	glm::mat4 getRotateMatrix() {
		...
		return (glm::eulerAngleYXZ(glm::radians(rotVec.y), glm::radians(rotVec.x), glm::radians(rotVec.z)));   // yaw, pitch, roll 
	}
	glm::mat4 getTranslateMatrix() {
		glm::vec3 posVec = static_cast<glm::vec3>(position);
		return (glm::translate(glm::mat4(1.0), glm::vec3(posVec.x, posVec.y, posVec.z)));
	}
	glm::mat4 getScaleMatrix() {
		glm::vec3 scaleVec = static_cast<glm::vec3>(scale);
		return (glm::scale(glm::mat4(1.0), glm::vec3(scaleVec.x, scaleVec.y, scaleVec.z)));
	}
	...
	//
	ofParameter<glm::vec3> position = glm::vec3(0, 0, 0);   // translate
	ofParameter<glm::vec3> rotation = glm::vec3(0, 0, 0);   // rotate
	ofParameter<glm::vec3> scale = glm::vec3(1, 1, 1);      // scale
...
public:
	Cone(ofColor color = ofColor::blue) : SceneObject() {
		diffuseColor = color;
	}
	Cone(glm::vec3 tran, glm::vec3 rot, glm::vec3 sc, ofColor color = ofColor::blue) : SceneObject() {
		position = tran;
		rotation = rot;
		scale = sc;
		diffuseColor = color;
...
public:
	Cube(ofColor color = ofColor::blue) : SceneObject() {
		diffuseColor = color;
	}
	Cube(glm::vec3 tran, glm::vec3 rot, glm::vec3 sc, ofColor color = ofColor::blue) : SceneObject() {
		position = tran;
		rotation = rot;
		scale = sc;
		diffuseColor = color;
...
public:
	Sphere(glm::vec3 p, float r, ofColor diffuse = ofColor::lightGray) : SceneObject() {
		position = p;
		radius = r;
...
public:
	Plane(glm::vec3 p, glm::vec3 n, ofColor diffuse = ofColor::darkGreen, float w = 20, float h = 20 ) : SceneObject() {
		position = p; normal = n;
		width = w;
		height = h;
		diffuseColor = diffuse;
